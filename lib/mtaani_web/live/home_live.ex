defmodule MtaaniWeb.HomeLive do
  ...
  def handle_info({:save_post, content, temp_id}, socket) do
    ...
    case Mtaani.Social.create_post(attrs) do
      ...
      {:error, _changeset} ->
        pending_posts = Enum.map(socket.assigns.pending_posts, fn
...
def handle_event("create_post", %{"content" => content}, socket) do
  if String.trim(content) != "" do
    ...
    temp_post = %{
      ...
      pending: true,
      failed: false      # ADD THIS LINE
    }
    
    pending_posts = [temp_post | socket.assigns.pending_posts]
    socket = assign(socket, pending_posts: pending_posts, new_post_content: "", show_new_post_modal: false)
...
def handle_event("retry_post", %{"temp_id" => temp_id}, socket) do
  ...
  def render(assigns) do
    ...
    <div class="h-full flex flex-col bg-gradient-to-b from-onyx/5 to-white">
              ...
              <div class="feed-post bg-white rounded-xl shadow-sm border border-onyx-mauve/10 overflow-hidden opacity-90">
                <div class="p-4 flex items-center justify-between">
                  <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full bg-verdant-forest/20 flex items-center justify-center">
                      <span class="text-verdant-forest font-semibold"><%= String.slice(pending.user.name, 0..0) %></span>
                    </div>
                    <div>
                ...
                <div class="px-4 pb-2">
                  <%= if pending.failed do %>
                    <div class="flex items-center gap-2 text-red-500 text-sm">
                      ...
                      <button phx-click="retry_post" phx-value-temp_id={pending.id} class="text-verdant-forest underline">Retry</button>
                      <button phx-click="delete_pending_post" phx-value-temp_id={pending.id} class="text-onyx-mauve underline">Discard</button>
                    </div>
                  <% else %>
            ...
            <%= for post <- @posts do %>
              <div class="feed-post bg-white rounded-xl shadow-sm border border-onyx-mauve/10 overflow-hidden">
                ...
                <div id={"comment-section-#{post.id}"} class="hidden comment-section">
                  <div class="px-4 pb-3">
                    <div class="flex gap-2">
                      <input type="text" placeholder="Write a comment..." class="flex-1 px-4 py-2 rounded-full bg-gray-100 dark:bg-gray-800 focus:outline-none focus:ring-1 focus:ring-verdant-forest" />
                      <button class="send-comment bg-verdant-forest text-white rounded-full px-4 py-2 text-sm hover:bg-verdant-deep transition-colors">Post</button>
                    </div>
                  </div>
                </div>
                ...
                <div class="px-4 py-2 flex justify-between items-center">
                  <!-- Comment Button -->
                  <button 
                    class="comment-button flex items-center gap-2 text-onyx-mauve hover:text-verdant-forest transition-colors group"
                  ...
                  <!-- Repost Button -->
                  <button class="flex items-center gap-2 text-onyx-mauve hover:text-verdant-sage transition-colors group">
                    <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button 
                    ...
                    class="flex items-center gap-2 text-onyx-mauve hover:text-red-500 transition-colors group"
                  >
                    <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <!-- Share Button -->
                  <button class="flex items-center gap-2 text-onyx-mauve hover:text-verdant-forest transition-colors group">
                    <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Create Post Modal -->
      <div id="new-post-modal" :if={@show_new_post_modal} class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl max-w-md w-full modal-content">
          ...
          <form phx-submit="create_post" phx-change="update_new_post">
            ...
            <div class="p-4 border-t flex justify-end">
              <button type="submit" class="bg-verdant-forest text-white px-6 py-2 rounded-full hover:bg-verdant-deep transition-colors">
                Post
              </button>
            </div>
          </form>
