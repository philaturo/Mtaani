defmodule MtaaniWeb.HomeLive do
  ...
  def handle_event("retry_post", %{"temp_id" => temp_id}, socket) do
    ...
    case Enum.find(socket.assigns.pending_posts, &(&1.id == temp_id_int)) do
      %{content: content} ->
        pending_posts = Enum.map(socket.assigns.pending_posts, fn
          %{id: ^temp_id_int} = pending -> %{pending | retrying: true}
          other -> other
        end)
          ...
          <% else %>
            ...
            <%= for post <- @posts do %>
              <div class="feed-post bg-white rounded-xl shadow-sm border border-onyx-mauve/10 overflow-hidden">
                ...
                <div class="px-4 py-1 flex flex-wrap gap-1 border-b border-gray-100">
  <%= for {emoji, count} <- get_reaction_counts(post.id) do %>
    <button 
      phx-click="add_reaction" 
      phx-value-type="post" 
      phx-value-id={post.id} 
      phx-value-emoji={emoji}
      class="inline-flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded-full text-sm hover:bg-gray-200 transition-colors"
    >
      <span><%= emoji %></span>
      <span class="text-xs text-onyx-mauve"><%= count %></span>
    </button>
  <% end %>
</div>
...
<div class="px-4 py-2 flex">
  <button 
    ...
    phx-value-id={post.id} 
    phx-value-emoji="❤️"
    class="flex-1 flex items-center justify-center gap-2 py-2 text-onyx-mauve hover:text-red-500 transition-colors"
  >
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
    </svg>
  ...
  <button class="flex-1 flex items-center justify-center gap-2 py-2 text-onyx-mauve hover:text-verdant-forest transition-colors">
    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
    </svg>
    <span>Comment</span>
